...
TUI 테스트는 일반 실행과 구형 CLI 확장 목록 양쪽에서 연속 두 번 `/reload`해도 두 계정 행이 유지되는지 확인한다. 오류 테스트는 Bad Request, Daybreak, context window, 인증·접근·rate-limit 및 stream 전 예외의 같은 계정 재시도, 3회 총 시도 제한, 중간 error 차단, 원래 context 유지, 계정 2 직접 호출, 부분 출력·취소 시 재실행 차단을 검증한다. 오프라인 테스트는 소진/초기화/오래된 캐시, quota에서만 계정 전환, streaming 후·취소 시 재실행 금지, 두 토큰 분리, 중복 계정 거부, 두 계정 footer, thinking 보존을 검증한다. 실제 두 계정 로그인과 한도 소진 E2E는 별도로 확인해야 한다. 한도를 테스트하려고 실제 사용량을 소진하지 않는다.
