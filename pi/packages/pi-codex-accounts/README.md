...
- 계정 2·3은 브라우저에서 각각 **다른 ChatGPT 계정**으로 승인한다. 어느 슬롯이든 기존 슬롯과 같은 계정이면 저장 전에 거부한다.
- OAuth의 브라우저 callback/paste-code 흐름은 Pi 기본 UI를 사용한다. 여러 로그인 과정을 동시에 시작하지 않는다(기본 callback 포트 공유).
- 계정 1 재인증: `/login openai-codex`. 계정 2·3 해제: `/logout`에서 해당 `Codex 계정` 선택.
- 모델은 계속 `openai-codex/gpt-6-sol`를 선택한다. 계정 2·3 provider는 로그인 슬롯이며 별도 모델 목록을 만들지 않는다.
...
```text
/codex-accounts use 3
/codex-accounts use 2
/codex-accounts use 1
```

연결된 계정만 선택할 수 있다. 선택은 `codex-accounts/preference.json`에 `0600`으로 저장되며 새 세션과 재시작 후에도 유지된다. 변경된 코드를 로드한 기존 세션도 **다음 모델 요청부터** 새 선택을 확인한다. 패치된 `pi-codex-image-gen@0.1.12`의 `codex_generate_image`는 다음 도구 호출부터 이 **저장된 우선 계정**을 사용한다. 텍스트 모델의 quota 자동 전환으로 프로세스에서만 활성화된 계정은 전역 우선 선택을 변경하지 않으며 이미지 도구도 이를 따라가지 않는다. 진행 중인 요청은 취소하거나 재실행하지 않는다. 구버전이 실행 중이면 먼저 `/reload`한다. 우선 계정이 소진됐다면 다른 계정으로 자동 전환하는 규칙은 그대로다.
...
6. 새 Pi 프로세스는 저장된 우선 계정부터 시작한다(선택 이력이 없으면 계정 1). 소진 후 자동 전환한 프로세스는 해당 계정을 유지하되, 사용자가 다시 `use 1|2|3`으로 지정하면 다음 요청부터 그 선택을 우선한다.
...
확인된 계정 usage/quota 소진을 제외한 **모든 Codex 요청 오류**는 같은 계정·같은 모델 요청으로 즉시 최대 2회 재시도한다(최초 포함 최대 3회). 여기에는 `Bad Request`, Daybreak 접근 확인, context window 초과, 네트워크·인증·접근·일시 rate-limit 오류와 stream 생성 전 예외가 포함된다. 중간 실패는 바깥 agent에 전달하지 않으며, 계속 실패하면 재시도 횟수를 표시하고 멈춘다. 확인된 quota 소진만 기존 규칙대로 다른 계정으로 전환한다.

아직 start/content/tool-call 이벤트를 하나도 내보내지 않았고 실패 응답에도 부분 출력이 없어야 한다. 취소는 재시도하지 않는다. 완료한 도구 결과가 포함된 동일 context를 다시 전송할 뿐, 파일 변경·명령·배포·이미지 생성 도구를 다시 실행하지 않는다. Context window 초과는 같은 입력을 보내므로 대개 세 번 모두 실패하며, 그때는 `/compact` 또는 입력 축소가 필요하다.

계정 2·3 alias로 직접 복원된 요청도 같은 라우터를 거친다. 코드 수정만으로 실행 중 프로세스가 바뀌지는 않으며, 기존 Pi는 작업이 끝난 뒤 `/reload`해야 적용된다.
...
~/.pi/agent/codex-accounts/3.json
```
...
`https://chatgpt.com/backend-api/wham/usage`의 읽기 전용 GET으로 세 계정을 조회한다. 여러 세션이 있어도 계정별 poll lease와 캐시로 대체로 60초마다 한 번만 조회한다(세션은 15초마다 캐시 확인). 수동 갱신과 quota 확인은 더 자주 조회할 수 있다. endpoint가 실패/변경되면 조회 실패로 표시하며 사용량을 추측하지 않는다. 모델 답변 요청은 사용량 조회 목적으로 만들지 않는다.
