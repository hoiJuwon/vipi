...
```text
Thinking: high  Weekly Usage Limit: 1* 주45% · 2 5h80%/주90%
```

숫자는 **남은 사용량**, `*`는 현재 Pi 프로세스가 마지막으로 요청을 보낸 계정이다. 계정 2 로그인 전에는 `2 미연결`로 표시한다. `~`는 오래되었거나 조회에 실패한 마지막 값이고, `?`는 reset 시각이 지나 재조회가 필요한 값이다. 제공되지 않는 5시간 창은 억지로 만들지 않는다. `/codex-accounts`는 새 조회를 시도하고 각 창의 초기화 시각도 보여준다. 잔여량은 실시간 보장이 아니라 최근 서버 관측값이다.
