...
The initially inferred topic remains fixed during automatic naming and status refreshes, but an explicit full rename may replace it. Tree `r` requires `분류 / 내용`; `/name 분류 / 내용` also replaces the topic, while a summary-only `/name` keeps it. The naming request has a 30-second timeout. Provider errors (including resolved responses with `stopReason: error`) produce a warning, not a truncated-first-message title. An unnamed session retries its first user request on the next turn. `/retitle` explicitly regenerates an existing title from the first user request on the current branch, preserving the topic; manual renames and session switches during a request take precedence. Existing titles are not silently rewritten. Run `node scripts/test-session-title.mjs` from the repository root for offline regression checks.
