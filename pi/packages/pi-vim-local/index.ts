...
export class ModalEditor extends CustomEditor {
  ...
  private pendingConversationNav: boolean = false;
  ...
  private statusLineFn: (status: { text: string; mode: Mode }) => void = () => {};
  private lastStatusLineKey = "";
  ...
  private sessionTabFn: (direction: 1 | -1, count?: number) => void = () => {};
  private reviewFn: () => void = () => {};
  private conversationNavFn: (target: "top" | "bottom" | "toggle") => void = () => {};
  ...
  setSessionTabFn(fn: (direction: 1 | -1, count?: number) => void): void {
    this.sessionTabFn = fn;
  }
  setReviewFn(fn: () => void): void {
    this.reviewFn = fn;
  }
  setConversationNavFn(fn: (target: "top" | "bottom" | "toggle") => void): void {
    this.conversationNavFn = fn;
  }
  setStatusLineFn(fn: (status: { text: string; mode: Mode }) => void): void {
    this.statusLineFn = fn;
    this.emitStatusLine();
  }
  ...
  private setMode(mode: Mode = "insert"): void {
    ...
    this.mode = mode;
    // Leaving insert ends any host-tainted session, so a later Insert
    // re-entry records normally again.
  ...
  private prepareRepeatRecordingForInput(key: string): void {
    ...
    if (this.mode === "insert") {
      if (this.shouldCancelInsertRepeatInput(key)) {
        ...
        this.discardUndoWindow();
        // Enter submits and opens a fresh prompt in NORMAL, so a later Insert
        // re-entry is repeatable again. Tab-completion is a host edit inside the
      ...
      // Implicit insert (startup or host-driven re-entry): the editor is already
      // in insert mode with no recording open, so a vim `i`/`a`/… entry never
  ...
  private clearPendingState(): void {
    ...
    this.pendingG = false;
    this.pendingGCount = "";
    this.pendingConversationNav = false;
  ...
  handleInput(data: string): void {
    // app.clear (Ctrl+C by default) is normally consumed by CustomEditor before
    // modal input sees it. Handle it here so it is exactly equivalent to Esc
    // and never clears the composed prompt.
    if (this.modalKeybindings.matches(data, "app.clear")) {
      this.handleEscape();
      return;
    }
    try {
      // Terminals may coalesce quickly typed Normal-mode keys into one printable
      // chunk (for example "gt", "gT", or "2gt"). Dispatch those grapheme by
      // grapheme so Vim sequences never depend on typing speed. Insert and EX
      // input deliberately keep their original whole-chunk behavior.
      if (this.mode !== "insert" && this.pendingExCommand === null && isPrintableChunk(data)) {
        const graphemes = getLineGraphemes(data);
        if (graphemes.length > 1) {
          for (const grapheme of graphemes) this.handleInputCore(data.slice(grapheme.start, grapheme.end));
          return;
        }
      }
  ...
  private handleInputCore(data: string): void {
    ...
    try {
      // Vim users commonly use Ctrl+C as a quicker Escape. Intercept it
      // before CustomEditor handles app.interrupt/app.clear so the draft is
      // preserved and every mode follows the same transition as Escape.
      if (isEscapeLikeInput(data) || matchesKey(data, Key.ctrl("c"))) {
        this.handleEscape();
        return;
      }
      ...
      if ("insert" === this.mode) {
        ...
        const submitEnter = this.isSubmitEnterInput(data);
        const hadSubmittableText = submitEnter && this.getText().trim().length > 0;
        super.handleInput(data);
        // The host clears the editor only for a real submit. Autocomplete
        // confirmation and newline insertion retain text and stay in INSERT.
        if (hadSubmittableText && this.getText() === "") {
          this.setMode("normal");
        }
        return;
      }
  ...
  private handleNormalMode(data: string): void {
    if (this.pendingConversationNav) {
      this.pendingConversationNav = false;
      if (data === "t" || data === "b" || data === "z") {
        this.clearPendingState();
        this.conversationNavFn(data === "t" ? "top" : data === "b" ? "bottom" : "toggle");
        return;
      }
      this.clearPendingState();
      return;
    ...
    if (this.pendingG) {
      ...
      const hadGCount = gCount.length > 0;
      this.pendingGCount = "";

      if (data === "t" || data === "T") {
        const rawCount = gCount || this.prefixCount;
        const count = rawCount ? Math.max(1, Math.min(MAX_COUNT, Number.parseInt(rawCount, 10))) : undefined;
        this.clearPendingState();
        this.sessionTabFn(data === "t" ? 1 : -1, count);
        return;
      }
    ...
    if (data === "z") {
      this.pendingConversationNav = true;
      return;
    }
  ...
  render(width: number): string[] {
    // Keep text aligned after a two-cell left margin plus a two-cell Normal-mode prompt.
    this.setPaddingX(4);
    ...
    const plain = (line: string) => line
      ...
      .replace(/\x1b\[[0-?]*[ -/]*[@-~]/gu, "")
      .replace(/\x1b_[^\x1b]*(?:\x1b\\)/gu, "");
    const isBorder = (line: string) => {
      const text = plain(line);
      return (text.match(/─/gu)?.length ?? 0) >= Math.max(4, Math.floor(width / 2));
    };
    const borders = lines
      .map((line, index) => isBorder(line) ? index : -1)
      .filter((index) => index >= 0);
    const topBorder = borders[0] ?? -1;
    const bottomBorder = borders[1] ?? -1;
    const editorLines = topBorder >= 0 && bottomBorder > topBorder
      ? lines.slice(topBorder + 1, bottomBorder)
      : lines.filter((line) => !isBorder(line));
    const trailingLines = bottomBorder >= 0 ? lines.slice(bottomBorder + 1) : [];

    if (this.mode !== "insert") {
      for (let index = 0; index < editorLines.length; index++) {
        let line = editorLines[index]!;
        const markerIndex = line.indexOf(CURSOR_MARKER);
        if (markerIndex >= 0) {
          const before = line.slice(0, markerIndex);
          const after = line.slice(markerIndex + CURSOR_MARKER.length);
          const softwareCursor = after.match(/^\x1b\[7m([^\x1b]*)\x1b\[0m/u);
          line = softwareCursor
            ? `${before}${softwareCursor[1] ?? ""}${after.slice(softwareCursor[0].length)}`
            : `${before}${after}`;
        }
        editorLines[index] = line.replaceAll(CURSOR_MARKER, "");
      }
    }
    if (editorLines.length > 0) {
      const greenPrompt = "  \x1b[1;38;2;130;253;172m> \x1b[22;39m";
      editorLines[0] = editorLines[0]!.replace(/^ {4}/u, greenPrompt);
    }

    // One visually empty row above and below the actual editor content.
    const paddingRow = "\u2800".repeat(Math.max(1, width - 1));
    const contentLines = [paddingRow, ...editorLines, paddingRow];
    const backgroundStart = "\x1b[48;2;24;24;24m";
    const backgroundEnd = "\x1b[49m";
    for (let index = 0; index < contentLines.length; index++) {
      const line = contentLines[index]!.replaceAll("\x1b[0m", `\x1b[0m${backgroundStart}`);
      contentLines[index] = `${backgroundStart}\x1b[2K${line}${backgroundEnd}`;
    }
    lines.splice(0, lines.length, ...contentLines, ...trailingLines);
  ...
  private emitStatusLine(): void {
    const rawStatus = this.getBottomStatusLine();
    const pendingNormal = this.mode === "normal" && !rawStatus ? this.getModeLabel().trim() : "";
    const footerText = rawStatus || pendingNormal;
    const statusKey = `${this.mode}\u0000${footerText}`;
    if (statusKey === this.lastStatusLineKey) return;
    this.lastStatusLineKey = statusKey;
    this.statusLineFn({ text: footerText, mode: this.mode });
  }

  private getBottomStatusLine(): string {
    if (this.pendingExCommand !== null) return `${this.pendingExCommand}█`;
    if (this.mode === "insert") return "-- INSERT --";
    if (this.mode === "visual") return "-- VISUAL --";
    if (this.mode === "visual-line") return "-- VISUAL LINE --";
    return "";
  }

  private getBottomStatusColorizer(): ((s: string) => string) | null {
    if (!this.borderColorizers) return null;
    return this.borderColorizers[this.getActiveMode()] ?? null;
  }
  ...
  private getModeLabel(): string {
    ...
    if (this.pendingConversationNav) return " NORMAL z_ ";
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("session_start", (_event, ctx) => {
    ...
    ctx.ui.setEditorComponent((tui, theme, kb) => {
      ...
      // Publish and run the configured Insert hook for the editor's initial
      // mode as well as later transitions (for example, initial Korean IME).
      modeChangeHandler("insert", "insert");
      editor.setSessionTabFn((direction, count) => {
        pi.events.emit("pi-vim:session-tab", { direction, count });
      });
      editor.setReviewFn(() => {
        pi.events.emit("pi-vim:review", {});
      });
      editor.setConversationNavFn((target) => {
        pi.events.emit("pi-vim:conversation-nav", { target });
      });
      editor.setStatusLineFn((status) => {
        pi.events.emit("pi-vim:status-line", status);
      });
