# pi-vim (local UI fork)

Local changes on top of `pi-vim` 0.14.1:

- `Ctrl+C` follows the same mode transition as `Esc`, preserving prompt text.
- Vim mode and EX command state render on a dedicated bottom-left row.
- EX input renders as `:command█`; NORMAL mode leaves the row blank.
...
The prompt is a borderless dark input area with one visually empty padding row above and below the actual text. Every mode shows a green `> ` after a two-cell left margin. NORMAL, VISUAL, and V-LINE hide the editor cursor; INSERT keeps the same text alignment and shows the cursor. Hit `Esc` to enter the modal modes. A successful typed `Enter` submission also returns directly to NORMAL; empty Enter, autocomplete confirmation, and multiline insertion stay in INSERT. Motions (`w`, `f{char}`, `%`, `25gg`), operators with counts (`3dw`, `ci"`, `ya{`), text objects, and `.` to repeat the last change. Undo is scoped like vim's: one `u` reverts one whole change — an insert session, a `cw`, a paste — and `<C-r>` brings it back. The boundaries (block visual, macros, search, vim ex semantics) are documented below rather than half-built.
...
| `gt` / `{count}gt` | Next Pi session or the numbered session through `pi-session-tree` |
| `gT` / `{count}gT` | Previous Pi session or move backward by count through `pi-session-tree` |
| `zt` / `zb` | Jump to the top/bottom of the latest assistant response through `pi-conversation-nav` |
| `zz` | Toggle between the latest assistant response top and bottom |
...
Typing done in the initial implicit insert session is repeatable too: the prompt opens in insert mode, so the first keystroke records an `i…<Esc>` change even though no `i` was pressed. A successful submit (`<Enter>`) returns to NORMAL and is never part of the recording, so a later `.` re-types the run but never resubmits.
...
pi-vim does not bundle any such tool and does not care which one you use — any shell command works. This installation intentionally leaves `modeChange` unset. `switch-ime.sh` is retained as a no-op so Pi processes that loaded an older command cannot continue changing any computer's IME. In an SSH + tmux setup the IME belongs to the remote keyboard Mac, is global rather than pane-local, and a server-triggered SSH change measured 170–416 ms even with multiplexing. That delay lets Vim mode and IME state diverge and background sessions can interfere with the active keyboard, so server-side automatic IME switching is not considered safe here.
